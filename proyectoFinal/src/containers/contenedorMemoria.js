class ContenedorMemoria {
    ...
    getCartProducts = (req, res) => {
        try {
            ...
            if (indexObj == -1) {
                return res.status(404).json({code: 404, msg: `Carrito ${id} no encontrado`});
            } else {
            ...
            console.log(error);
            return res.status(500).json({code: 500, msg: `Error al obtener los productos del carrito mediante el metodo ${req.method}`});
        }
    }
    ...
    newCart = (req, res) => {
        try {
            ...
            return res.status(201).json({msg: 'Carrito creado con exito!'}); 
        } catch (error) {
            console.log(error);
            return res.status(500).json({code: 500, msg: `Error al crear un nuevo carrito mediante el metodo ${req.method}`});
        } 
    }
    ...
    addCartProduct = (req, res) => {
        try {
            ...
            if (indexObj == -1 || indexProd == -1) {
                return res.status(404).json({code: 404, msg: `El carrito ${id} o el producto ${req.body.id} no existen`});
            } else {
                this.carritos[indexObj].productos.push(this.productos[indexProd]);
                return res.status(201).json({msg: `Producto agregado con exito al carrito!`});
            }
        } catch (error) {
            console.log(error);
            return res.status(500).json({code: 500, msg: `Error al agregar un producto en el carrito mediante el metodo ${req.method}`});
        } 
    }
    ...
    deleteCart = (req, res) => {
        try {
            ...
            if (indexObj == -1) {
                ...
                res.status(201).json({msg: `Carrito ${id} eliminado!`});
            }
        } catch (error) {
            console.log(error)
    ...
    deleteCartProduct = (req, res) => {
        try {
            ...
            const indexObj = this.carritos.findIndex((item)=> item.id == id);
            if (indexObj == -1)  {
                return res.status(404).json({code: 404, msg: `El carrito ${id} no existe`});
            } else {
                let prods = this.carritos[indexObj].productos;
                const indexProd = prods.findIndex((item)=> item.id == id_prod);
                if (indexProd == -1) {
                    return res.status(404).json({code: 404, msg: `Producto no encontrado`});
                } else {
                    prods.splice(indexProd, 1);
                    return res.status(201).json({msg: `Producto eliminado con exito del carrito!`});
                }
            }
        } catch (error) {
            console.log(error);
            return res.status(500).json({code: 500, msg: `Error al eliminar el producto del carrito mediante el metodo ${req.method}`});
        }
    }
    ...
    getProduct = (req, res) => {
        try {
            if (req.params.id) {
                ...
                res.status(200).json(this.productos[indexObj]);
            } else {
